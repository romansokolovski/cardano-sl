...
spec = describe "Wallets" $ do
    describe "CreateWallet" $ do
        describe "Wallet creation (wallet layer)" $ do
            ...
            prop "fails if the wallet already exists" $ withMaxSuccess 50 $ \nm -> do
                monadicIO $ do
                    pwd     <- genSpendingPassword
                    request <- WalletLayer.CreateWallet <$> genNewWalletRq pwd
                    withLayer nm $ \layer _ -> do
            ...
            prop "supports Unicode characters" $ withMaxSuccess 1 $ \nm -> do
                monadicIO $ do
                    pwd     <- genSpendingPassword
        ...
        describe "Wallet creation (Servant)" $ do
            ...
            prop "comes by default with 1 account at a predictable index" $ withMaxSuccess 50 $ \nm -> do
                monadicIO $ do
                    pwd <- genSpendingPassword
                    rq  <- genNewWalletRq pwd
                    withLayer nm $ \layer _ -> do
            ...
            prop "comes by default with 1 address at the default account" $ withMaxSuccess 50 $ \nm -> do
                monadicIO $ do
                    pwd <- genSpendingPassword
                    rq  <- genNewWalletRq pwd
                    withLayer nm $ \layer _ -> do
    ...
    describe "DeleteWallet" $ do
        describe "Wallet deletion (wallet layer)" $ do
            ...
            prop "cascade-deletes all the associated accounts" $ withMaxSuccess 50 $ \nm -> do
                monadicIO $ do
                    withNewWalletFixture nm $ \_ layer _ Fixture{..} -> do
                        let wId = V1.walId fixtureV1Wallet
                        liftIO $ do
            ...
            prop "fails if the wallet doesn't exists" $ withMaxSuccess 50 $ \nm -> do
                monadicIO $ do
                    wId  <- pick arbitrary
                    withLayer nm $ \layer _ -> do
                        liftIO $ do
    ...
    describe "UpdateWalletPassword" $ do
        ...
        describe "Wallet update password (wallet layer)" $ do
            ...
            prop "fails if the old password doesn't match" $ withMaxSuccess 50 $ \nm -> do
                monadicIO $ do
        ...
        describe "Wallet update password (kernel)" $ do
            ...
            prop "correctly updates hdRootHasPassword" $ \nm -> do
                monadicIO $ do
                    newPwd <- pick arbitrary
                    withNewWalletFixture nm $ \ _ _ wallet Fixture{..} -> do
                        res <- Kernel.updatePassword wallet
                                                     fixtureHdRootId
                                                     (unV1 fixtureSpendingPassword)
                                                     newPwd
    ...
    describe "GetWallet" $ do
        ...
        describe "Get a specific wallet (wallet layer)" $ do
            ...
            prop "fails if the wallet doesn't exist" $ withMaxSuccess 50 $ \nm -> do
                monadicIO $ do
                    wId <- pick arbitrary
                    withLayer nm $ \ layer _ -> do
                            res <- WalletLayer.getWallet layer wId

...
getOwnUtxoForPk genesisData ourPk = getOwnUtxos genesisData ourAddresses
  where
    getNM :: GenesisData -> NetworkMagic
    getNM = makeNetworkMagic . gpcProtocolMagic . gdProtocolConsts
    --
    ourAddresses :: [Address]
    ourAddresses =
        map (flip (makePubKeyAddress (getNM genesisData)) ourPk . IsBootstrapEraAddr) [False, True]
